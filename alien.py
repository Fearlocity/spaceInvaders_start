...
class Alien(Sprite): 
    alien_images = [[pg.transform.rotozoom(pg.image.load(f'images/alien_03-{n}.png'), 0, 2) for n in range(2)],
                    [pg.transform.rotozoom(pg.image.load(f'images/alien__1{n}.png'), 0, 2) for n in range(2)],
                    [pg.transform.rotozoom(pg.image.load(f'images/alien__2{n}.png'), 0, 2) for n in range(2)]]
    ...
    def hit(self):
        if not self.dying:
            ...
            self.sb.calculate_score(self.type)
    def update(self): 
...
class Aliens:
    ...
    def update(self): 
        if self.check_fleet_bottom(): self.game.game_over()
        if self.check_fleet_edges(): self.change_fleet_direction()
...
class Mystery(Sprite):
    def __init__(self, game):
        ...
        self.image = pg.image.load(f'images/alien_03-0.png')
        ...
        self.mysteryEntered = pg.mixer.Sound('sounds/mysteryentered.wav')
        self.mysteryEntered.set_volume(0.3)
        self.playSound = True
        self.mystery = Group()

    def check_collisions(self):
        collisions = pg.sprite.groupcollide(self.mystery, self.ship_lasers, False, True)  
        if collisions:
    ...
    def update(self, currentTime):
        ...
        if passed > self.moveTime:
            if (self.rect.x < 0 or self.rect.x > 1200) and self.playSound:
                self.mysteryEntered.play()
                self.playSound = False
            if self.rect.x < 1240 and self.direction == 1:
                self.mysteryEntered.play()
                self.rect.x += 2
